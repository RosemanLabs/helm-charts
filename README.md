...
Due to the large number of values that need to be overwritten, we recomend deploying it by running the `deploy_single_node.sh` script as follows. With these steps, you can setup your own VDL node:

1. Acquire the needed key material (discuss with your RosemanLabs contact on how to do this) and store all in a single directory
2. Create your overwrite file, a template can be found below. Be sure to set the right values.
